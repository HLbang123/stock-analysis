...
- R01 子信号现状：对子顶(5)/巨量见顶(4,量比≥4x升5)/第二波见顶(4)/涨停炸板(4)/RSI超买新高(2)/巨量异动(1)。**形态类见顶(长上影/长下影/跳空衰竭/纺锤线)与涨停封板已全部删除**（生产胜率+十年回放反复证伪；封板 T+5 +1.48% 是强势延续）。RSI超买新高=价格创新高+前摆动高 RSI(14)≥70，先落弱提醒待复验。
...
- **`docs/alert-rules.md`** — 人工参考文档，与代码同步维护（表格 + 正文常有两处要改）。
- **⚠️ 规则说明弹窗只有一个 tab 会自动同步**（2026-08-15 踩过）：`AlertRulesModal.tsx` 三 tab 里只有「预警规则」tab 自动读 `ALERT_RULES`；「市场扫描」(ScanDoc) 和「波段打分」(TScoreDoc) 是**手写静态文案**，改扫描条件/阶段预设/t-score 因子后必须手动改这两处，代码不会带着它们走。
...
- **2026-08-26**：R01 新增「RSI超买新高」弱提醒(sev2)子信号（价格创新高+前摆动高 RSI(14)≥70；十年回放负向，先弱提醒待确认日复验）。深度分析对 60日涨幅≥30% 且逼近 MA60 的标的加压力提示。
