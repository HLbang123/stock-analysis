...
import {
  ...
  DEFAULT_DRAGON_FIRST_YIN_CONFIG,
  DragonBar,
...
function processRaw(raw: RawBar[], exclusiveLowerDate: string): { rows: Row[]; detail: DetailRow[] } {
  ...
  const defaultCfg: Partial<DragonFirstYinConfig> = {};
  const permissiveCfg: Partial<DragonFirstYinConfig> = {
    ...DEFAULT_DRAGON_FIRST_YIN_CONFIG,
    yinBodyMaxPct: 999,
    maxVolumeRatio: 999,
    maxYinTurnoverRate: 999,
  ...
  for (const [code, rawBars] of rawByCode) {
    ...
    const signals = scanDragonFirstYinSignals(bars, defaultCfg);
    ...
    // 明细候选：用宽松阈值记录所有「结构上成立」的首阴，供离线参数扫描
    for (let i = DEFAULT_DRAGON_FIRST_YIN_CONFIG.minBoards; i < bars.length; i++) {
      const sig = detectDragonFirstYinAt(bars, i, permissiveCfg);
      if (!sig.yin || !sig.run) continue;
      const structuralFail = sig.failedChecks.some((f) =>
        ['no_limit_up_run', 'not_a_yin', 'board_count_out_of_range', 'high_board_not_fake_yin'].includes(f)
      );
      if (structuralFail) continue;
      if (sig.yin.date <= fmtDate(exclusiveLowerDate)) continue;
      const next = bars[sig.yin.index + 1];
      if (!next) continue;
      const entryClose = sig.yin.close;
      ...
      const changeBoards = sig.run.boards.filter((b) => !b.oneWord);
      const minChangeBoardTurnover = changeBoards.length > 0
      ...
      detail.push({
        ...
        retCloseHigh,
        retBoardHigh,
        changeCount: sig.run.changeCount,
        oneWordCount: sig.run.oneWordCount,
        trailingOneWordStreak: sig.run.trailingOneWordStreak,
        minChangeBoardTurnover: minChangeBoardTurnover != null && Number.isFinite(minChangeBoardTurnover) ? minChangeBoardTurnover : null,
        bodyPct: sig.yin.bodyPct,
        changePct: sig.yin.changePct,
        volumeRatio: sig.yin.volumeRatio,
        turnoverRate: sig.yin.turnoverRate,
        atLimitDown: sig.yin.atLimitDown,
        isHighBoard: sig.run.boardCount >= DEFAULT_DRAGON_FIRST_YIN_CONFIG.highBoardThreshold,
      });
    }
  }
  return { rows, detail };
...
async function main() {
  ...
  for (let start = marginDays; start < datesAsc.length; start += chunkDays, chunkIdx += 1) {
    ...
    const { rows, detail } = processRaw(raw, exclusiveLower);
    allRows.push(...rows);
    allDetail.push(...detail);
    console.log('chunk', chunkIdx, 'window', exclusiveLower, '->', windowEnd, 'loaded', raw.length, 'signals', rows.length, 'detail', detail.length, 'elapsed', Date.now() - t0, 'ms');
  }

  writeFileSync('/tmp/dragon-first-yin-detail.json', JSON.stringify(allDetail));
  console.log('detail saved:', allDetail.length);
