...
const CHANGELOG: ChangeEntry[] = [
  {
    date: '2026-09-14',
    items: [
      '扫描页「分层筛选」升级为「每日推荐」：从主板里挑「最近确实在急跌、又没被炒到最热」的——先取近20日跌幅榜前列，再要求近5日跌超5%且成交未极度萎缩，据此避开「阴跌中继」；每天一次模型分析，写明推荐理由与反面证据',
      '「每日推荐」新增按日期回看：左右箭头切换前后一天，点日历图标可展开按月挑选',
    ],
  },
  {
    date: '2026-09-09',
    items: [
      '板块口径统一：板块行情与筛选采用同一套概念/行业指数',
    ],
  },
  {
    date: '2026-09-07',
  ...
  },
  {
    date: '2026-08-26',
    items: [
      '复盘新增日历视图：按月回看行情状态，量能冰点日标记，点日期可看当日详情',
      '深度分析更谨慎：近期涨幅较大的标的逼近关键均线时，会提示短期回落风险',
      '预警新增「短期过热」弱提醒：标的创新高且指标过热时，追加一条风险提示',
    ],
  },
  {
    date: '2026-08-21',
    items: [
      'AI 筛选打分调优：修复稳健优选策略分数整体偏低的问题，移除趋势策略的均线多头排列强制条件，新增 5/13 金叉加分',
      '修复 API 密钥失效、过期或余额不足时深度分析静默降级的问题：现在会直接提示原因，不再展示不完整结果',
    ],
  },
  {
    date: '2026-08-19',
    items: [
      '自选分组支持拖拽排序：在「管理分组」弹窗按住把手拖动，即可调整分组顺序',
      '修复五日线附近的两处误报：从高位跌回误报「站稳五日线」、低位反弹误报「跌破5日线」',
      'AI 筛选更审慎：新增规则分门槛、收紧入选数，低分标的不再凑数，行情偏弱时宁缺毋滥',
    ],
  },
  {
    date: '2026-08-18',
    items: [
      '详情页日K线优化',
      'ETF 预警与波段打分按自身波动率自动调整幅度阈值：低波品种不再误报，高波品种不再迟钝',
      'ETF 详情页新增折溢价提醒：跨境/商品类溢价超 2% 时提示',
      '分享详情支持整组添加：可多选对方的分组一键加为自己的分组，同名分组自动合并',
      '修复组内添加标的时，已在自选的标的被标「已添加」、无法加入本组的问题',
    ],
  },
  {
    date: '2026-08-17',
    items: [
      '全站提速：自选、分享、预警检测改批量加载，自选较多时不再卡顿',
      '首页预警清空后仍可切换分组查看',
      '均线多头排列：预警改用更灵敏的短期均线，市场扫描新增 5/10/13、5/10/20 两种组合',
      '买入信号调整：箱体信号升为强信号，反包入场、均线多头排列移出买入共振；移除涨停封板提醒（保留炸板预警）与两个无效见顶形态',
      '分组优化：组内底部可直接添加标的（勾选/搜索/截图识别），修复分组太多时选择列表滑不动的问题',
      '分享优化：详情支持多选/全选、整组一键移入自选；修复内容偶发多出无关标的；多台设备共用同一分享码；对方撤销后订阅标记「已撤销」；新增订阅人数显示',
      'AI 对话新增查询工具：自选分组与持仓、预警信号、主力资金、股东户数、吸筹箱体状态，问「我的自选/持仓/预警」即可直接结合分析',
    ],
  },
  {
    date: '2026-08-15',
  ...
  },
  {
    ...
    items: [
      ...
      'AI 筛选新增板块+主板过滤:结果可按行业/创业板/科创板/北交所切片查看',
  ...
  },
  {
    ...
    items: [
      ...
      '扫描器优化：金叉新增「即将金叉」、55日线改为价格在55日线上方、板块动态化（全行业）、修复首次查询空结果bug',
