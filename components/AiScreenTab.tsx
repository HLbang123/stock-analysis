...
import { ChevronDown, ChevronUp, Sparkles, AlertTriangle, Loader2 } from 'lucide-react';
...
export function AiScreenTab() {
  ...
  const latestRunMeta = runs.find((r) => r.strategyId === selected);
  const updatedLabel = current
    ? `${current.barDate.slice(4, 6)}-${current.barDate.slice(6, 8)} 已更新`
    : '';
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {strategies.map((s) => {
          ...
          return (
            ...
            >
              {active && updatedLabel && (
                <span className="absolute top-2.5 right-3 text-[10px] text-gray-400">{updatedLabel}</span>
              )}
              ...
              {s.rulesText && (
                <div className="text-[10px] text-gray-400 mt-2 whitespace-pre-line leading-relaxed opacity-80">{s.rulesText}</div>
              )}
            </button>
          );
        })}
      ...
      {current && (
        <Card className="p-3 mb-4">
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
            ...
            {latestRunMeta?.llmModel && <span className="text-gray-400">{latestRunMeta.llmModel}</span>}
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-2">每日 16:00 自动运行，稍后打开查看</p>
        </div>
      )}
    </div>
  );
