...
本仓库已建 CodeGraph 索引(`.codegraph/`)。定位代码、理解调用关系、查影响面时,优先用
`codegraph explore "<问题或符号名>"`(等价于 MCP 的 codegraph_explore),
或 `codegraph query <符号>` / `codegraph callers <符号>` / `codegraph impact <符号>`。
