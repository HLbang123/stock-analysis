...
| R04 | 妇联定律 | 工业富联sh601138涨>8%→科技股警惕 | CRITICAL | A |
| R05 | 5/13金叉 | MA5上穿MA13；放量+站上55日线→强买(A级)，否则谨慎(B级) | INFO | A |
| R06 | 5/10金叉 | MA5上穿MA10；放量确认短线启动 | INFO | B |
| R07 | 止跌企稳 | 抛压>10%+新低区锤子线(下影≥实体×2+上影<1%)或十字星 | INFO | A |
| R08 | RSI超卖 | RSI(6)<20超卖，强下行趋势(MA20下行+破MA20)中过滤 | INFO | A |
| R09 | 反包入场 | 15日回调>5%+涨≥5%+收>98%近期高 | INFO | B |
| R10 | 黄金反弹 | 回调至0.382-0.618+涨>3%+放量×1.2 | INFO | B |
| R11 | 箱体信号 | 突破(40日振幅<20%+突破>3%+放量)优先，或吸筹(60日箱体+放量小阳) | INFO | B |
| R12 | 多头排列 | MA5>MA13>MA55+股价站上MA55+放量，且刚刚形成 | INFO | B |
| R13 | 站稳五日线加仓 | 连续3日收盘站上MA5+MA5上行+温和放量+站稳刚形成 | INFO | B |
| R14 | 筹码低位密集 | 90%集中度<0.18+获利盘>60%+主峰≤现价×1.03，结构偏多（换手率转移模型，参考级，不计入共振）| INFO | B |
| R15 | 筹码高位套牢 | 获利盘<40%+主峰>现价×1.05+20日涨>15%，上方套牢重（参考级，不计入共振）| WARNING | B |`;
...
引擎初判（参考，15条规则检测结果）：${engineResults}`;
}
...
## 你的专属技术规则库
- **RSI超卖**：RSI(6)<20，且非强下行趋势(MA20下行+破MA20) → 超卖/买入信号（R08，A级信号；底背离已移除，底部由R07止跌企稳覆盖）
- **箱体信号**：40日箱体突破>3%+放量，或60日箱体放量小阳吸筹 → 趋势启动信号（R11，B级）
- **离场阶梯**：破趋势线/破五日线/放量离场/缩量破位/急跌/5/10死叉/5/13死叉等破位离场信号合并阶梯，择优返回一条，死叉清仓覆盖前序减仓 → 硬止损信号必须提醒（R02，A级）
- **均线多头排列**：MA5>MA13>MA55+站上55日线且刚形成 → 多头格局确立（R12，B级）
- **站稳五日线加仓**：连续3日收盘站上MA5+MA5上行+站稳刚形成 → 强势确立加仓信号（R13，B级）
...
- **趋势底线**：离场阶梯(R02)触发死叉清仓档 或跌破55日线(R03)→趋势转弱，不抢反弹；5/13金叉(R05)放量+站上55日线才是有效买点。
...
- **强确认档**：若多重买入类硬信号共振（规则引擎中 R05/R06/R09/R10/R11/R13 等≥2 条触发），按"强烈看多"处理，仓位可至 1/2；仅单一信号触发仍按温和看多≤1/3
